...
/// Factory to create EntityManagers - similar to org.apache.openjpa.persistence.EntityManagerFactory
...
/// EntityManager - main interface to load and persist entities -- similar to org.apache.openjpa.persistence.EntityManager
abstract class AbstactEntityManager
{
    /// returns metadata
    ...
    /// Update the persistent instance changed columns with the identifier of the given detached instance.
    void updateChanges(Object obj);

    /// remove object from DB (renamed from original EntityManager.delete - it's keyword in D)
...
class EntityManager : AbstactEntityManager {
    ...
    private EntityManagerAccessor _accessor;
    @property EntityManagerAccessor accessor() {
        return _accessor;
    }
    ...
    this(EntityManagerFactory managerFactory, EntityMetaData metaData, Dialect dialect, DataSource connectionPool) {
        ...
        this._accessor = new EntityManagerAccessor(this);
    }
    ...
    override Connection close() {
        ...
        closed = true;
        managerFactory.managerClosed(this);
    ...
    override void updateChanges(Object obj)
    {
        auto info = metaData.findEntityForObject(obj);
        enforceEx!TransientObjectException(info.isKeySet(obj), "Cannot persist entity w/o key assigned");
        ///TODO changed save
    ...
    // renamed from EntityManager.delete since delete is D keyword
...
//    EntityManagerFactoryObserver observer;
    private bool closed;
    private EntityMetaData metaData;
    Dialect dialect;
    DataSource connectionPool;
...
//    this(Configuration cfg, Mapping mapping, Settings settings, EventListeners listeners, EntityManagerFactoryObserver observer) {
//        this.cfg = cfg;
...
class QueryImpl : Query
{
    ...
    private Object readRelations(Object objectContainer, DataSetReader r, PropertyLoadMap loadMap) {
        ...
        for (int i = 0; i < query.select.length; i++) {
            ...
            if (!from.entity.isKeyNull(r, from.startColumn)) {
                ...
                row = manager.peekFromCache(from.entity.name, key);
                if (row is null) {
                    ...
                    manager.metaData.readAllColumns(row, r, from.startColumn);
                    manager.putToCache(from.entity.name, key, row);
            ...
            for (int j=0; j<ei.length; j++) {
                ...
                if (pi.oneToOne || pi.manyToOne) {
                    ...
                    if (rfrom !is null && rfrom.selectIndex >= 0) {
                        ...
                        if (pi.columnName !is null) {
                            ...
                            if (r.isNull(from.startColumn + pi.columnOffset)) {
                                ...
                                Object existing = manager.peekFromCache(pi.referencedEntity.name, id);
                                if (existing !is null) {
                                    ...
                                    if (pi.lazyLoad) {
                                        ...
                                        LazyObjectLoader loader = new LazyObjectLoader(manager.accessor, pi, id);
                    ...
                    if (pi.lazyLoad) {
                        ...
                        LazyCollectionLoader loader = new LazyCollectionLoader(manager.accessor, pi, id);
    ...
    private void delayedLoadRelations(PropertyLoadMap loadMap) {
        ...
        foreach(pi; types) {
            ...
            if (pi.oneToOne || pi.manyToOne) {
                if (pi.columnName !is null) {
                    Variant[] unknownKeys;
                    Object[] list = manager.lookupCache(pi.referencedEntity.name, map.keys, unknownKeys);
                    if (unknownKeys.length > 0) {
                        ...
                        QueryImpl q = cast(QueryImpl)manager.createQuery(hql);
    ...
    Object[] listObjects(Object placeFirstObjectHere, PropertyLoadMap loadMap) {
        ...
        enforceEx!EntityManagerException(ei !is null, "No entity expected in result of query " ~ getQueryString());
