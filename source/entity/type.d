...
/// Thrown when the user passes a transient instance to a EntityManager method that expects a persistent instance. 
...
/// Thrown when the user calls a method of a EntityManager that is in an inappropropriate state for the given call (for example, the the session is closed or disconnected). 
class EntityManagerException : HibernatedException {
    this(string msg, string f = __FILE__, size_t l = __LINE__) { super(msg, f, l); }
    this(Throwable causedBy, string f = __FILE__, size_t l = __LINE__) { super(causedBy.msg, f, l); }
}
